...
export default function Watchlist(){
  ...
  async function load(){
    const r = await axios.get(`${API}/api/symbols`);
    setRows(r.data || []);
  }
  ...
  async function del(c: string){
    await axios.delete(`${API}/api/symbols`, { params: { code: c } });
    await load();
  }
  ...
  async function order(c: string, side: 'buy'|'sell', qty: number, price: number){
    try{
      ...
      if(r.ok){
        alert(side.toUpperCase() + ' 응답: ' + (d?.status || '') + ' / ' + (d?.client_id || ''));
      }else{
        alert('오류: ' + JSON.stringify(d));
      }
    }catch(e:any){
